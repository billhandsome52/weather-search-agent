...
from app.agent import get_agent_chain
...
async def startup_event():
    ...
    global agent_chain
    try:
        agent_chain = get_agent_chain(verbose=settings.verbose)
        print("✅ Agent 初始化成功")
...
async def chat(request: ChatRequest):
    ...
    try:
        result = agent_chain.invoke(request.message, session_id)
        return ChatResponse(
...
async def health_check():
    """健康检查"""
    return {
        "status": "ok" if agent_chain else "unavailable",
        "llm_provider": settings.llm_provider,
        "has_search_key": settings.has_search_key,
    }
